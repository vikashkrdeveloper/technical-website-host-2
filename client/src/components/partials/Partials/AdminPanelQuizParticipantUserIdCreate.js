...
function AdminPanelQuizParticipantUserIdCreate() {
    ...
    const QuizUserdata = async (emailid) => {
        try {
            setError(false);
            SetIsLoading(true);
            const res = await fetch(`/api/participant/quiz/register/data/filter?search=${emailid}`, {
                method: 'POST'
            })
            if (res.status === 200) {
    ...
    const submidformdata = async (event) => {
        ...
        try {
            ...
            const res = await fetch('/participant/quiz/user-id/tech/form', {
                method: 'POST',
                body: formdata
            })
            if (res.status === 200) {
...
export default AdminPanelQuizParticipantUserIdCreate
